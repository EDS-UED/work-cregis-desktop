...
export type EdsInspectCatalogEntry = {
  ...
  domClass?: string;
  /**
   * 外壳 chrome 借名（全局 `canBorrowShellSubtreeName`，见 inspectIdentity.ts）。
   * 禁止 overlay 整壳 flatten；Popover/Detail 内 DS 子组件走 `findNearestVueCatalogOwner`。
   */
  shellSubtree?: boolean | 'overlay' | 'chrome';
  /** 仅 Vue DOM 根可 owner（DataListColumn td、FlotationMenu 浮层根等）。 */
  ownerDomRootOnly?: boolean;
  props: EdsPropSpec[];
};
...
export function isShellSubtreeCatalogEntry(entry: EdsInspectCatalogEntry): boolean {
  return Boolean(entry.domClass && entry.shellSubtree);
}

export function isDomRootOnlyCatalogEntry(entry: EdsInspectCatalogEntry): boolean {
  return Boolean(entry.ownerDomRootOnly);
}
